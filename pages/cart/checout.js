...
Page({
  ...
  onClicktjButton: function () {
    ...
    var supplierid = wx.getStorageSync("pos");
    var supplierid = supplierid["street_id"];
    if (that.data.fromtype == 'cart') {
      //提交订单
      App._post('order/cart_pay', { supplier: supplierid}, function (result) {
        console.log('success');
        //这里发起支付
  ...
  wx_pay_fun: function (Rdata) {
    ...
    wx.requestPayment({
      ...
      success: function (res) {
        ...
        console.log(Rdata);
        wx.request({
          url: 'https://static.aishangnuanjia.com/addons/litestore/api.order/huidiao',
          data: {
