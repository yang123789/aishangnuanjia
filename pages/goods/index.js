...
Page({
  ...
  onLoad: function (options) {
    ...
    //添加足迹
    App._post('user/footprint', { goods_id: that.data.goods_id }, function (result) {
    ...
    //这里获得购物车数量
    App._get('cart/getTotalNum2', {}, function (result) {
      that.setData( {
  ...
  coll:function(e){
    ...
    var that=this;
    App._post('user/collection', { goods_id:id}, function (result) {
      console.log(result)
      if (result.code==1){
        wx.showToast({
          title: '收藏成功',
  ...
  qxcoll: function (e) {
    var id = e.currentTarget.dataset.id;
    var that = this;
    App._post('user/collectiondel', { goods_id: id }, function (result) {
      console.log(result)
      if (result.code == 1) {
        wx.showToast({
          title: '取消成功',
